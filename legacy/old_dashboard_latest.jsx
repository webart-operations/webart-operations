...
export default function DashboardView({ setActiveTab }) {
  ...
  const loadData = async () => {
    ...
    if (role === 'manager') {
       // Filter revenue by projects in that team
       const { data: teamProjs } = await supabase.from('projects').select('id').eq('team', profile.team);
       if (teamProjs && teamProjs.length > 0) revQuery = revQuery.in('project_id', teamProjs.map(p => p.id));
       else revQuery = revQuery.eq('project_id', '00000000-0000-0000-0000-000000000000');
    }
    ...
    if (role !== 'sales') {
      ...
      if (role === 'manager') {
        // Manager sees all staff in their team
        const { data: teamStaff } = await supabase.from('profiles').select('full_name').eq('team', profile.team);
        const teamNames = new Set((teamStaff || []).map(s => s.full_name));
        gridArray = gridArray.filter(g => teamNames.has(g.name));
      } else if (role === 'am') {
          ...
          return (
            <Card key={i} className="stat-card" flush>
              <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p className="stat-label" style={{ marginBottom: 4 }}>{c.label}</p>

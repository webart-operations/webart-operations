...
export default function AuditQueueView({ appSettings }) {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('full_name, role, team').eq('status', 'active')
      .then(({ data }) => {
        if (data) setStaff({ 
          ams: data.filter(d => ['am', 'manager', 'ceo', 'qa'].includes(d.role)).map(d => ({ name: d.full_name, team: d.team })), 
          pms: data.filter(d => ['pm', 'manager', 'ceo', 'qa'].includes(d.role)).map(d => ({ name: d.full_name, team: d.team })),
          qas: data.filter(d => ['qa', 'manager', 'ceo'].includes(d.role)).map(d => d.full_name)
        });
      });
  }, []);
...
function AuditFormModal({ submission, onClose, staff, profile, onComplete }) {
  ...
  useEffect(() => {
    if (isReadOnly) {
       supabase.from('qa_audits').select('*').eq('submission_id', submission.id).single()
  ...
  const handleSubmit = async (e) => {
    ...
    if (finalStatus === 'passed') {
      ...
      if (!mClientId) {
          // Check if client exists by email first to prevent duplicates
          const { data: existingC } = await supabase.from('clients').select('id').eq('email', submission.email).single();
          if (existingC) {
      ...
      if (newProj) await supabase.from('submissions').update({ project_id: newProj.id, client_id: mClientId }).eq('id', submission.id);
              ...
              <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', padding: 24, borderRadius: 'var(--radius-sm)', marginTop: 24 }}>
                ...
                <div className="grid-2">
                  ...
                  <Select label="Assign PM *" value={pm} onChange={e => setPm(e.target.value)} required>
                    <option value="">Unassigned</option>
                    {staff.pms.map(s => <option key={s.name} value={s.name}>{s.name} ({s.team})</option>)}
                  </Select>
                </div>
              </div>
            )}

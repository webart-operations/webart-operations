...
export default function ReactivationView({ appSettings }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 1. Fetch all clients and their projects (Restore previous logic)
      const { data: allClients } = await supabase.from('clients').select('*, projects(*)');
      
      // 2. Identify Reactivation Leads (Client-Centric)
      // A client is a lead if: 0 active projects AND at least 1 historical project
      const leads = (allClients || []).filter(c => {
        const projects = c.projects || [];
        const hasActive = projects.some(p => p.status === 'active' || p.status === 'at_risk');
        const hasHistorical = projects.some(p => ['on_hold', 'delivered', 'dead', 'closed'].includes(p.status));
        return !hasActive && hasHistorical;
      });
      setClients(leads);

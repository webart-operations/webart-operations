...
export default function DashboardView({ setActiveTab, appSettings }) {
  ...
  const calculateStats = (allSubs, allProjs, rev, queueCount) => {
    ...
    // Total Net Sales (Only from passed NON-reactivation audits)
    const passedSalesOnly = passedSubs.filter(s => !s.is_reactivation);
    const totalNet = passedSalesOnly.reduce((sum, s) => sum + Number(s.usd_net || s.net || 0), 0);
    const totalGross = passedSalesOnly.reduce((sum, s) => sum + Number(s.usd_gross || s.gross || 0), 0);
  ...
  const loadData = async () => {
    ...
    if (role !== 'sales') {
      ...
      filteredSubs.forEach(s => {
         ...
         if (s.is_reactivation && !(appSettings?.sales_reps || []).includes(s.closer)) return; // Exclude AM/PM reactivations from Sales Leaderboards
      ...
      (rev || []).forEach(r => {
         ...
         const logger = r.logged_by || 'Unknown';
